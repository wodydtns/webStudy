 ...
 * 1. 개발 단계
 *	1) HttpServlet을 상속받은 자바 객체 정의 -> 필요한 메서드 overriding
 ...
 *	2) web.xml에 서블릿을 등록할 때의 설정을 캡슐화해서 서블릿 하나당 servletConfig 객체를 1:1로 관리함.
 * 
 * 3. Servlet의 메서드(callback) 종류
 *	1) lifecycle callback : 생명주기 내에서 단 한번(singleton)만 호출
 *		ex) init, destroy
 *	2) request callback	: mapping URI로 방생하는 요청에 대해 반복 호출
 *		ex) service, doXXX
 */
